...
function sendObject(req, res, next, obj) {
  ...
  if (res.swagger.schema.type === 'array') {
    if (res.swagger.schema.items.allOf) {
      var alleDefs = res.swagger.schema.items.allOf
      for (const idx in alleDefs) {
        const currentSchema = alleDefs[idx]
        if (currentSchema.hasOwnProperty('properties') && currentSchema.properties.hasOwnProperty('href')) {
            obj = patchResult(obj, req)
          }
      }
    } else {
      if (res.swagger.schema.hasOwnProperty('items') && res.swagger.schema.items.hasOwnProperty('properties') && res.swagger.schema.items.properties.hasOwnProperty('href')) {
        obj = patchResult(obj, req)
      }
    }
  } else {
    obj = patchObject(obj, req)
  }
...
function patchResult(inObject, req) {
  for (var itemIdx in inObject) {

    if (inObject[itemIdx].hasOwnProperty('id')) {
      if (req.path.endsWith("/")) {
        inObject[itemIdx].href = req.path + inObject[itemIdx].id;
      } else {
        inObject[itemIdx].href = req.path + '/' + inObject[itemIdx].id;
      }
    }
...
function patchObject(inObject, req) {
  if (inObject.hasOwnProperty('id')) {
    if (req.path.endsWith("/")) {
      inObject.href = req.path + inObject.id;
    } else {
      if (!req.path.endsWith(inObject.id)) {
        inObject.href = req.path + '/' + inObject.id;
      } else {
        inObject.href = req.path
      }
    }
  }
  return inObject
