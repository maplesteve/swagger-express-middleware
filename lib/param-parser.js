...
function parseParameter(param, value, schema) {
  ...
  // Check for 'readOnly' properties in the body
  if (param.hasOwnProperty("in") && param.in === "body") {
    for (const valueItem in value) {
      if (schema.properties.hasOwnProperty(valueItem) &&
          schema.properties[valueItem].hasOwnProperty("readOnly") &&
          schema.properties[valueItem].readOnly) {

        throw ono({status: 400}, 'Parameter "%s" is marked readOnly but was part of the request body.', valueItem);
      }
    }
  }
